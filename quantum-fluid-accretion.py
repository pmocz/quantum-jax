...
def bh_bondi_step_ambient(
    ...
    kappa_sink=2.0, ann_lo=2.0, ann_hi=4.0
):
    nx = rho.shape[0]
    ...
    dM_cap = jnp.minimum(dM, M_avail_kernel) if (fmax is None) else jnp.minimum(dM, fmax * M_avail_kernel)
...
def compute_step(psi, rho, vx, vy, vz, t, M_bh_acc, x_bh, vxbh, vybh, vzbh, bh_placed):
    ...
    if BH_ON and BH_MOVE:
        ...
        V_hat_dm_g_pre, _ = get_potential_bg(jnp.abs(psi)**2 + (rho if GAS_SELF_GRAV else 0.0), rho_bg)
    ...
    if BH_ON:
        rho_try, dMdt_try, rho_inf_try, r_B_try, dM_cap = bh_bondi_step_ambient(
            ...
            r_acc_mult=r_acc_mult_eff, lam=LAMBDA_ISO, fmax=BH_FMAX, kappa_sink=2.0, ann_lo=2.0, ann_hi=4.0
        )
        ...
        dMdt = jnp.array(0.0); rho_inf = jnp.array(rho_gas); r_B = jnp.array(0.0)
    ...
    if BH_ON and BH_MOVE:
        active = jnp.where(bh_placed > 0.5, 1.0, 0.0)
        V_hat_dm_g2, _ = get_potential_bg(jnp.abs(psi)**2 + (rho if GAS_SELF_GRAV else 0.0), rho_bg)
...
def update(_, state):
    (psi, rho, vx, vy, vz, t,
     ...
     dMdt, rho_inf, r_B, bh_placed) = compute_step(
...
def _write_video_side_by_side(frames_left, frames_right, marks_left, marks_right,
    ...
    imL = axs[0].imshow(np.log10(frames_left[0].T + 1e-30), origin="lower",
                        vmin=np.log10(vminL + 1e-30), vmax=np.log10(vmaxL + 1e-30),
                        extent=(0, W, 0, H), cmap=cmap_left)
    imR = axs[1].imshow(np.log10(frames_right[0].T + 1e-30), origin="lower",
                        vmin=np.log10(vminR + 1e-30), vmax=np.log10(vmaxR + 1e-30),
...
def main():
    ...
    state["x_bh"] = jnp.mod(state["x_bh"] + jnp.array([sx*dx, sy*dx, sz*dx], dtype=state["x_bh"].dtype), Lx)
...
if __name__ == "__main__":
    ...
    fig_ts, axs = plt.subplots(2, 2, figsize=(11.5, 7.5), dpi=120, constrained_layout=True)
    ...
    axL.set_xlim(0.0, xmax); axR.set_xlim(0.0, xmax)
    ...
    try:
        ...
        if Pdm.ndim == 2 and Pgas.ndim == 2 and Pdm.shape == Pgas.shape and rc is not None and len(Pdm) > 0:
            n = Pdm.shape[0]
    ...
    fig_v, axv = plt.subplots(1, 1, figsize=(6.6, 4.2), dpi=130, constrained_layout=True)
    ...
    # --- baseline vs boost ---
    rho0 = float(rho_gas); lam = float(LAMBDA_ISO); Gf = float(G); cs = float(cs_const)
    dMdt0_g = (4.0 * np.pi * lam * (Gf**2) * (M_bh_plot**2) * rho0 / (cs**3 + 1e-300)) / T_GYR_PER_UNIT
    alpha = np.where(dMdt0_g > 0.0, dMdt_g / dMdt0_g, np.nan)

    fig_boost, axb = plt.subplots(1, 1, figsize=(6.5, 4.0), dpi=120, constrained_layout=True)
    ...
    combos = [
        ("dm_xy", "g_xy",  "bh_xy", os.path.join(out_dir, "combo_xy6.mp4"), cmr.bubblegum, "viridis"),
        ("dm_xz", "g_xz",  "bh_xz", os.path.join(out_dir, "combo_xz6.mp4"), cmr.bubblegum, "viridis"),
        ("dm_yz", "g_yz",  "bh_yz", os.path.join(out_dir, "combo_yz6.mp4"), cmr.bubblegum, "viridis"),
    ]
    ...
    for dm_key, gas_key, mark_key, path, cmap_dm, cmap_gas in combos:
        ...
        _write_video_side_by_side(frames_dm[:n], frames_gas[:n], marks[:n], marks[:n],
                                  path, cmap_left=cmap_dm, cmap_right=cmap_gas, fps=fps, dpi=110)
